...
export default function LoginPage() {
  ...
  return (
    <div className='flex flex-col min-h-dvh p-6 pt-12'>
      {/* Logo */}
      <div className='mb-8 flex items-center gap-2'>
        <Coffee size={20} className='text-[#D4956A]' />
        <span className='text-sm font-semibold text-[#A89080] tracking-wider'>yemo°</span>
      </div>

      {/* Heading */}
      <div className='mb-8'>
        <h1 className='font-display text-[28px] font-bold text-[#2C1A0E] leading-tight'>
          Welcome<br />back.
        </h1>
        <p className='text-sm text-[#A89080] mt-2'>Sign in to continue your order</p>
      </div>
      ...
      <form onSubmit={handleLogin} className='flex flex-col gap-4 flex-1'>
        <div>
          ...
          <input
            ...
            required
            className='w-full h-11 px-4 bg-[#F2EAE1] border-2 border-[#6B3F2A] rounded-xl text-sm text-[#2C1A0E] placeholder:text-[#A89080] outline-none focus:border-[#D4956A] transition-colors'
          />
        </div>
        ...
        <button type='submit' disabled={loading}
          className='h-12 bg-[#6B3F2A] text-[#FDFAF6] rounded-2xl text-sm font-bold mt-2 active:scale-95 transition-transform disabled:opacity-60'>
        ...
        <Link href='/home'
          className='h-10 border-2 border-[#E2DDD8] rounded-2xl text-sm font-semibold text-[#A89080] flex items-center justify-center gap-2 active:bg-[#F2EAE1] transition-colors'>
          👋 Continue as Guest
        </Link>

        <p className='text-center text-xs text-[#A89080] mt-auto pt-4'>
          New here?{' '}
